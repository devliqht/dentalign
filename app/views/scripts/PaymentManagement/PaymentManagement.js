class PaymentManagement {
    ...
    bindEvents() {
        // Page level events
        document.getElementById('refreshDataBtn').addEventListener('click', () => this.loadAppointments());
        ...
        document.getElementById('searchPatient').addEventListener('input', () => this.applyFilters());

        // Payment modal events
        document.getElementById('addItemBtn').addEventListener('click', () => this.addPaymentItem());
        document.getElementById('savePaymentBtn').addEventListener('click', () => this.savePayment());
        document.getElementById('deletePaymentBtn').addEventListener('click', () => this.deletePayment());
        document.getElementById('markAsPaidBtn').addEventListener('click', () => this.updatePaymentStatus('Paid'));
        document.getElementById('markAsPendingBtn').addEventListener('click', () => this.updatePaymentStatus('Pending'));
    ...
    async loadAppointments() {
        try {
            ...
            const response = await fetch('/dentalign/doctor/get-all-appointments-payments');
    ...
    renderAppointmentsTable() {
        ...
        tbody.innerHTML = this.filteredAppointments.map(appointment => {
            ...
            const totalAmount = appointment.TotalAmount ? `₱${parseFloat(appointment.TotalAmount).toFixed(2)}` : '$0.00';
    ...
    populateFilterOptions() {
        ...
        const doctors = [...new Set(this.appointments.map(app => app.DoctorName).filter(Boolean))];
    ...
    applyFilters() {
        ...
        this.filteredAppointments = this.appointments.filter(appointment => {
            // Status filter
            if (statusFilter && appointment.PaymentStatus !== statusFilter) return false;
            
            // Doctor filter
            if (doctorFilter && appointment.DoctorName !== doctorFilter) return false;
            
            // Search filter
            if (searchTerm && !appointment.PatientName.toLowerCase().includes(searchTerm)) return false;
                ...
                switch (dateFilter) {
                    ...
                    case 'month':
                        if (appointmentDate.getMonth() !== now.getMonth() || 
                            appointmentDate.getFullYear() !== now.getFullYear()) return false;
                        break;
                    case 'year':
    ...
    async openAddPaymentModal(appointmentId) {
        try {
            const response = await fetch(`/dentalign/doctor/get-payment-details?appointment_id=${appointmentId}`);
            const data = await response.json();
            ...
            if (data.success) {
                ...
                this.showToast('Error loading appointment details: ' + data.message, 'error');
            }
        } catch (error) {
    ...
    async openEditPaymentModal(appointmentId, paymentId) {
        try {
            const response = await fetch(`/dentalign/doctor/get-payment-details?payment_id=${paymentId}`);
            const data = await response.json();
            ...
            if (data.success && data.payment) {
                ...
                this.showToast('Error loading payment details: ' + data.message, 'error');
            }
        } catch (error) {
    ...
    populateModal() {
        ...
        document.getElementById('modalPatientName').textContent = appointmentData.PatientName || 'Unknown';
        document.getElementById('modalDoctorName').textContent = appointmentData.DoctorName || 'Unknown';
        document.getElementById('modalDateTime').textContent = new Date(appointmentData.DateTime || appointmentData.AppointmentDateTime).toLocaleString();
        document.getElementById('modalAppointmentType').textContent = appointmentData.AppointmentType || 'Unknown';

        // Populate payment info
        if (this.currentPayment) {
            document.getElementById('paymentStatus').value = this.currentPayment.Status || 'Pending';
            document.getElementById('paymentNotes').value = this.currentPayment.Notes || '';
        ...
        this.updateQuickActionButtons(document.getElementById('paymentStatus').value);
    ...
    renderPaymentItems() {
        ...
        container.innerHTML = this.paymentItems.map((item, index) => `
    ...
    updatePaymentItem(index, field, value) {
        if (index >= 0 && index < this.paymentItems.length) {
            ...
            this.paymentItems[index].Total = 
                (this.paymentItems[index].Amount || 0) * (this.paymentItems[index].Quantity || 1);
    ...
    async savePayment() {
        try {
            ...
            const validItems = this.paymentItems.filter(item => 
    ...
    async updatePaymentItems(items) {
        ...
        const currentItemIds = items.map(item => item.PaymentItemID).filter(Boolean);
        const originalItemIds = this.currentPayment.items.map(item => item.PaymentItemID);
    ...
    async updatePaymentStatus(status) {
        ...
        try {
            ...
            if (data.success) {
                ...
                this.showToast('Error updating payment status: ' + data.message, 'error');
            }
        } catch (error) {

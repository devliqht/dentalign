document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateTimeSlots() {
        ...
        if (doctorId && date) {
            timeSlotsContainer.innerHTML = '<div class="w-full text-center py-8"><p class="text-gray-500 text-sm">Loading available time slots...</p></div>';
                        ...
                        if (data.success && data.timeSlots && data.timeSlots.length > 0) {
                            ...
                            data.timeSlots.forEach(timeSlotData => {
                                ...
                                if (timeSlotData.available) {
                                    button.className = 'glass-card px-4 py-2 text-sm font-medium text-gray-700 border border-gray-200 rounded-lg hover:bg-nhd-brown/85 hover:text-white hover:border-nhd-brown transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-nhd-brown focus:ring-offset-2';
                                    button.addEventListener('click', function() {
                                        selectTimeSlot(timeSlotData.time, button);
                                    });
                                } else {
                                    button.className = 'glass-card px-4 py-2 text-sm font-medium text-gray-400 bg-gray-100 border border-gray-300 cursor-not-allowed rounded-lg transition-all duration-200 disabled';
                                    button.disabled = true;
                                }
                                
                                button.setAttribute('data-time', timeSlotData.time);
                                button.setAttribute('data-available', timeSlotData.available);
                                button.innerHTML = formatTime(timeSlotData.time) + (timeSlotData.available ? '' : ' <span class="ml-1 text-xs">(Taken)</span>');
                                
                                timeSlotsContainer.appendChild(button);
                            });
                        } else {
                            timeSlotsContainer.innerHTML = '<div class="w-full text-center py-8"><p class="text-gray-500 text-sm">No available time slots for the selected date.</p></div>';
                        }
                    } catch (e) {
                        console.error('Failed to parse JSON:', e);
                        console.error('Raw text was:', text);
                        timeSlotsContainer.innerHTML = '<div class="w-full text-center py-8"><p class="text-red-500 text-sm">Error loading time slots.</p></div>';
                    }
                })
                .catch(error => {
            ...
            timeSlotsContainer.innerHTML = '<div class="w-full text-center py-8"><p class="text-gray-500 text-sm">Please select a doctor and date to view available time slots.</p></div>';
        }
    }

    function selectTimeSlot(timeSlot, button) {
        // Don't allow selection if the button is disabled
        if (button.disabled || button.getAttribute('data-available') === 'false') {
            return;
        }
        
        // Remove selected class from all time slot buttons
        const allTimeButtons = document.querySelectorAll('.time-slot-btn');
